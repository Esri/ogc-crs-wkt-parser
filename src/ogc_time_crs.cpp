...
bool ogc_time_crs :: is_kwd(const char * kwd)
{
   ...
   ogc_error *       err)
{
   ...
   if ( cs == OGC_NULL )
   {
      ...
      bad = true;
                            ...
                            axis_1, OGC_NULL, OGC_NULL, timeunit, OGC_NULL, err) )
   {
      bad = true;
   }
...
{
   _datum         = ogc_time_datum    :: destroy( _datum         );
   _base_crs      = ogc_base_time_crs :: destroy( _base_crs      );
   _deriving_conv = ogc_deriving_conv :: destroy( _deriving_conv );
}

ogc_time_crs * ogc_time_crs :: destroy(
   ogc_time_crs * obj)
{
   if ( obj != OGC_NULL )
   {
   ...
   ogc_error *       err)
{
   ...
   for (int i = start; i < end; i = next)
   {
      ...
      if ( ogc_base_time_crs::is_kwd(arr[i].str) )
      {
         if ( base_crs != OGC_NULL )
         {
