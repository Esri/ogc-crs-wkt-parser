...
const char * ogc_lenunit :: obj_kwd() { return OGC_OBJ_KWD_LENUNIT; }
const char * ogc_lenunit :: alt_kwd() { return OGC_OBJ_KWD_UNIT;    }

bool ogc_lenunit :: is_kwd(const char * kwd)
{
 ...
 * object from tokens
 */
ogc_lenunit * ogc_lenunit :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 2 tokens: LENUNIT[ "name", factor ...
    */
   if ( same < 2 )
   {
 ...
 * object from WKT
 */
ogc_lenunit * ogc_lenunit :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_lenunit * obj = OGC_NULL;
 ...
 * clone
 */
ogc_lenunit * ogc_lenunit :: clone(const ogc_lenunit * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_lenunit * ogc_lenunit :: clone() const
{
   ogc_vector * ids = ogc_vector :: clone( _ids );

   ogc_lenunit * p = create(_name,
                            _factor,
                            ids,
                            OGC_NULL);
   if ( p == OGC_NULL)
 ...
 */
bool ogc_lenunit :: is_equal(
   const ogc_lenunit * p1,
