...
const char * ogc_citation :: obj_kwd() { return OGC_OBJ_KWD_CITATION; }

bool ogc_citation :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_citation * ogc_citation :: create(
   const char * text,
   ogc_error *  err)
{
   ogc_citation * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_citation();
      if ( p == OGC_NULL )
      {
 ...
 * object from tokens
 */
ogc_citation * ogc_citation :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: CITATION[ "text" ...
    */
   if ( same < 1 )
   {
 ...
 * object from WKT
 */
ogc_citation * ogc_citation :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_citation * obj = OGC_NULL;
 ...
 * clone
 */
ogc_citation * ogc_citation :: clone(const ogc_citation * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_citation * ogc_citation :: clone() const
{
   ogc_citation * p = create(_text);
   return p;
}
 ...
 */
bool ogc_citation :: is_equal(
   const ogc_citation * p1,
 ...
 * compare
 */
bool ogc_citation :: is_identical(
   const ogc_citation * p1,
   const ogc_citation * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
