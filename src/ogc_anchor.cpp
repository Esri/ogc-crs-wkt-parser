...
const char * ogc_anchor :: obj_kwd() { return OGC_OBJ_KWD_ANCHOR; }

bool ogc_anchor :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_anchor * ogc_anchor :: create(
   const char * text,
   ogc_error *  err)
{
   ogc_anchor * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      ...
      p->_obj_type = OGC_OBJ_TYPE_ANCHOR;
      p->_visible  = true;
   }
 ...
 */
ogc_anchor * ogc_anchor :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: ANCHOR[ "text" ...
    */
   if ( same < 1 )
   {
 ...
 * object from WKT
 */
ogc_anchor * ogc_anchor :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_anchor * obj = OGC_NULL;
 ...
 * clone
 */
ogc_anchor * ogc_anchor :: clone(const ogc_anchor * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_anchor * ogc_anchor :: clone() const
{
   ogc_anchor * p = create(_text);
   return p;
}
