...
const char * ogc_remark :: obj_kwd() { return OGC_OBJ_KWD_REMARK; }

bool ogc_remark :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_remark * ogc_remark :: create(
   const char * text,
   ogc_error *  err)
{
   ogc_remark * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      ...
      ogc_string::unescape_str(p->_text, text, OGC_UTF8_TEXT_MAX);
      p->_obj_type = OGC_OBJ_TYPE_REMARK;
   }
 ...
 * object from tokens
 */
ogc_remark * ogc_remark :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: REMARK[ "text" ...
    */
   if ( same < 1 )
   {
 ...
 * object from WKT
 */
ogc_remark * ogc_remark :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_remark * obj = OGC_NULL;
 ...
 * clone
 */
ogc_remark * ogc_remark :: clone(const ogc_remark * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_remark * ogc_remark :: clone() const
{
   ogc_remark * p = create(_text);
   return p;
}
