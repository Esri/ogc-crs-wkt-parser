...
const char * ogc_scaleunit :: obj_kwd() { return OGC_OBJ_KWD_SCALEUNIT; }
const char * ogc_scaleunit :: alt_kwd() { return OGC_OBJ_KWD_UNIT;      }

bool ogc_scaleunit :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_scaleunit * ogc_scaleunit :: create(
   const char * name,
   double       factor,
   ogc_vector * ids,
   ogc_error *  err)
{
   ogc_scaleunit * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_scaleunit();
      if ( p == OGC_NULL )
      {
      ...
      p->_obj_type  = OGC_OBJ_TYPE_SCALEUNIT;
      p->_visible   = true;
      p->_unit_type = OGC_UNIT_TYPE_SCALE;
      p->_factor    = factor;
      p->_ids       = ids;
   }
 ...
 * object from tokens
 */
ogc_scaleunit * ogc_scaleunit :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 2 tokens: SCALEUNIT[ "name", factor ...
    */
   if ( same < 2 )
   {
 ...
 * object from WKT
 */
ogc_scaleunit * ogc_scaleunit :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_scaleunit * obj = OGC_NULL;
 ...
 * clone
 */
ogc_scaleunit * ogc_scaleunit :: clone(const ogc_scaleunit * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_scaleunit * ogc_scaleunit :: clone() const
{
   ogc_vector * ids = ogc_vector :: clone( _ids );

   ogc_scaleunit * p = create(_name,
                              _factor,
                              ids,
                              OGC_NULL);
   if ( p == OGC_NULL)
 ...
 */
bool ogc_scaleunit :: is_equal(
   const ogc_scaleunit * p1,
 ...
 * compare
 */
bool ogc_scaleunit :: is_identical(
   const ogc_scaleunit * p1,
   const ogc_scaleunit * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
