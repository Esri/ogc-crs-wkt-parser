...
const char * ogc_geod_datum :: obj_kwd() { return OGC_OBJ_KWD_GEOD_DATUM; }
const char * ogc_geod_datum :: alt_kwd() { return OGC_ALT_KWD_GEOD_DATUM; }

bool ogc_geod_datum :: is_kwd(const char * kwd)
{
 ...
 * object from tokens
 */
ogc_geod_datum * ogc_geod_datum :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   {
      if ( ogc_ellipsoid::is_kwd(arr[i].str) )
      {
         if ( ellipsoid != OGC_NULL )
         {
 ...
 * object from WKT
 */
ogc_geod_datum * ogc_geod_datum :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_geod_datum * obj = OGC_NULL;
 ...
 * clone
 */
ogc_geod_datum * ogc_geod_datum :: clone(const ogc_geod_datum * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_geod_datum * ogc_geod_datum :: clone() const
{
 ...
 */
bool ogc_geod_datum :: is_equal(
   const ogc_geod_datum * p1,
 ...
 * compare
 */
bool ogc_geod_datum :: is_identical(
   const ogc_geod_datum * p1,
   const ogc_geod_datum * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
