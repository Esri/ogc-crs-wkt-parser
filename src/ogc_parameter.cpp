...
const char * ogc_parameter :: obj_kwd() { return OGC_OBJ_KWD_PARAMETER; }

bool ogc_parameter :: is_kwd(const char * kwd)
{
 ...
 * object from WKT
 */
ogc_parameter * ogc_parameter :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_parameter * obj = OGC_NULL;
 ...
 * clone
 */
ogc_parameter * ogc_parameter :: clone(const ogc_parameter * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
 ...
 */
bool ogc_parameter :: is_equal(
   const ogc_parameter * p1,
   const ogc_parameter * p2)
{
   ...
   if ( !ogc_string :: is_equal( p1->name(),  p2->name()  ) ||
        !ogc_macros :: eq      ( p1->value(), p2->value() ) ||
 ...
 * compare
 */
bool ogc_parameter :: is_identical(
   const ogc_parameter * p1,
   const ogc_parameter * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
 ...
 * get ID count
 */
int ogc_parameter :: id_count() const
{
 ...
 * get the nth ID
 */
ogc_id * ogc_parameter :: id(int n) const
{
