   ...
   ogc_error *       err)
{
   ...
   for (int i = start; i < end; i = next)
   {
      ...
      if ( ogc_string::is_equal(arr[i].str, ogc_base_proj_crs::obj_kwd()) )
      {
         if ( base_crs != OGC_NULL )
         {
      ...
      if ( ogc_string::is_equal(arr[i].str, ogc_base_engr_crs::obj_kwd()) )
      {
         if ( base_crs != OGC_NULL )
         {
            ogc_error::set(err, OGC_ERR_WKT_DUPLICATE_BASE_CRS, obj_kwd());
      ...
      if ( ogc_string::is_equal(arr[i].str, ogc_angunit  ::obj_kwd()) ||
           ...
           ogc_string::is_equal(arr[i].str, ogc_scaleunit::alt_kwd()) )
      {
         if ( unit != OGC_NULL )
         {
