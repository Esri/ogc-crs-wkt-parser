 ...
 * create
 */
ogc_base_geod_crs * ogc_base_geod_crs :: create(
   const char *         name,
   ...
   ogc_error *          err)
{
   ...
   else if ( !ogc_utils::validate_cs(OGC_OBJ_TYPE_BASE_GEOD_CRS, cs,
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_base_geod_crs();
      if ( p == OGC_NULL )
      {
 ...
 * object from tokens
 */
ogc_base_geod_crs * ogc_base_geod_crs :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: BASEGEODCRS[ "name" ...
    */
   if ( same < 1 )
   {
 ...
 * clone
 */
ogc_base_geod_crs * ogc_base_geod_crs :: clone(const ogc_base_geod_crs * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_base_geod_crs * ogc_base_geod_crs :: clone() const
{
   ...
   ogc_base_geod_crs * p = create(_name,
                                   datum,
                                   primem,
                                   cs,
                                   axis_1,
 ...
 */
bool ogc_base_geod_crs :: is_equal(
   const ogc_base_geod_crs * p1,
