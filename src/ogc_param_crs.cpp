...
bool ogc_param_crs :: is_kwd(const char * kwd)
{
...
{
   _datum         = ogc_param_datum    :: destroy( _datum         );
   _base_crs      = ogc_base_param_crs :: destroy( _base_crs      );
   _deriving_conv = ogc_deriving_conv  :: destroy( _deriving_conv );
}

ogc_param_crs * ogc_param_crs :: destroy(
   ogc_param_crs * obj)
{
   if ( obj != OGC_NULL )
   {
   ...
   ogc_error *       err)
{
   ...
   ogc_base_param_crs * base_crs = OGC_NULL;
   ...
   /*---------------------------------------------------------
    * There must be 1 token: PARAMCRS[ "name" ...
    */
   if ( same < 1 )
   {
   ...
   for (int i = start; i < end; i = next)
   {
      ...
      if ( ogc_base_param_crs::is_kwd(arr[i].str) )
      {
         if ( base_crs != OGC_NULL )
         {
         ...
         {
            base_crs = ogc_base_param_crs::from_tokens(t, i, &next, err);
            if ( base_crs == OGC_NULL )
               bad = true;
         }
         continue;
   ...
   if ( !bad )
   {
      ...
      else
      {
         ...
         ogc_base_param_crs :: destroy( base_crs );
   ...
   size_t    buflen) const
{
   ...
   *buffer = 0;
   ...
   rc &= ogc_base_param_crs :: to_wkt(_base_crs,      buf_base_crs, opts, OGC_TBUF_MAX);
...
ogc_param_crs * ogc_param_crs :: clone() const
{
   ...
   {
      base_crs  = ogc_base_param_crs :: clone( _base_crs      );
   ...
   {
      datum     = ogc_param_datum :: clone( _datum  );
      p = create(_name,
                  datum,
                  cs,
                  axis_1,
   ...
   const ogc_param_crs * p2)
{
   ...
   if ( !ogc_string         :: is_equal( p1->name(),          p2->name()          ) ||
        ...
        !ogc_base_param_crs :: is_equal( p1->base_crs(),      p2->base_crs()      ) ||
   ...
   const ogc_param_crs * p2)
{
   ...
   if ( !ogc_string         :: is_equal    ( p1->name(),          p2->name()          ) ||
        ...
        !ogc_base_param_crs :: is_identical( p1->base_crs(),      p2->base_crs()      ) ||
