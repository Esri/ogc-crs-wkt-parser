...
const char * ogc_base_vert_crs :: obj_kwd() { return OGC_OBJ_KWD_BASE_VERT_CRS; }
   ...
   ogc_error *       err)
{
   ...
   else if ( !ogc_utils::validate_cs(OGC_OBJ_TYPE_BASE_VERT_CRS, cs,
 ...
 * object from tokens
 */
ogc_base_vert_crs * ogc_base_vert_crs :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   ogc_base_vert_crs *    obj     = OGC_NULL;
   ...
   /*---------------------------------------------------------
    * There must be 1 token: BASEVERTCRS[ "name" ...
    */
   if ( same < 1 )
   {
   ...
   {
      if ( ogc_string::is_equal(arr[i].str, ogc_vert_datum::obj_kwd()) )
      {
         if ( datum != OGC_NULL )
         {
 ...
 * object from WKT
 */
ogc_base_vert_crs * ogc_base_vert_crs :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_base_vert_crs * obj = OGC_NULL;
 ...
 * clone
 */
ogc_base_vert_crs * ogc_base_vert_crs :: clone(const ogc_base_vert_crs * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_base_vert_crs * ogc_base_vert_crs :: clone() const
{
   ogc_lenunit * u = reinterpret_cast<ogc_lenunit *>(_unit);

   ogc_vert_datum *  datum   = ogc_vert_datum :: clone( _datum   );
   ...
   ogc_base_vert_crs * p = create(_name,
                                 datum,
                                 cs,
                                 axis_1,
                                 lenunit,
 ...
 */
bool ogc_base_vert_crs :: is_equal(
   const ogc_base_vert_crs * p1,
...
bool ogc_base_vert_crs :: is_equal(
   const ogc_base_vert_crs * p) const
{
 ...
 * compare
 */
bool ogc_base_vert_crs :: is_identical(
   const ogc_base_vert_crs * p1,
   const ogc_base_vert_crs * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
