 ...
 * clone
 */
ogc_meridian * ogc_meridian :: clone(const ogc_meridian * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_meridian * ogc_meridian :: clone() const
{
   ogc_angunit * angunit = ogc_angunit :: clone( _angunit );

   ogc_meridian * p = create(_value,
                             angunit,
                             OGC_NULL);
   if ( p == OGC_NULL )
   {
 ...
 */
bool ogc_meridian :: is_equal(
   const ogc_meridian * p1,
 ...
 * compare
 */
bool ogc_meridian :: is_identical(
   const ogc_meridian * p1,
   const ogc_meridian * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
