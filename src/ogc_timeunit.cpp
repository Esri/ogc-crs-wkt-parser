...
const char * ogc_timeunit :: obj_kwd() { return OGC_OBJ_KWD_TIMEUNIT; }
const char * ogc_timeunit :: alt_kwd() { return OGC_OBJ_KWD_UNIT;     }

bool ogc_timeunit :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_timeunit * ogc_timeunit :: create(
   const char * name,
   double       factor,
   ogc_vector * ids,
   ogc_error *  err)
{
   ogc_timeunit * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_timeunit();
      if ( p == OGC_NULL )
      {
      ...
      p->_obj_type  = OGC_OBJ_TYPE_TIMEUNIT;
      p->_unit_type = OGC_UNIT_TYPE_TIME;
      p->_factor    = factor;
      p->_ids       = ids;
   }
 ...
 * object from tokens
 */
ogc_timeunit * ogc_timeunit :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 2 tokens: TIMEUNIT[ "name", factor ...
    */
   if ( same < 2 )
   {
 ...
 * object from WKT
 */
ogc_timeunit * ogc_timeunit :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_timeunit * obj = OGC_NULL;
 ...
 * clone
 */
ogc_timeunit * ogc_timeunit :: clone(const ogc_timeunit * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_timeunit * ogc_timeunit :: clone() const
{
   ogc_vector * ids = ogc_vector :: clone( _ids );

   ogc_timeunit * p = create(_name,
                             _factor,
                             ids,
                             OGC_NULL);
   if ( p == OGC_NULL)
 ...
 */
bool ogc_timeunit :: is_equal(
   const ogc_timeunit * p1,
 ...
 * compare
 */
bool ogc_timeunit :: is_identical(
   const ogc_timeunit * p1,
   const ogc_timeunit * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
