 ...
 * create
 */
ogc_base_param_crs * ogc_base_param_crs :: create(
   const char *       name,
   ...
   ogc_error *        err)
{
   ...
   else if ( !ogc_utils::validate_cs(OGC_OBJ_TYPE_BASE_PARAM_CRS, cs,
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_base_param_crs();
      if ( p == OGC_NULL )
      {
      ...
      p->_obj_type = OGC_OBJ_TYPE_BASE_PARAM_CRS;
 ...
 * object from tokens
 */
ogc_base_param_crs * ogc_base_param_crs :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   ogc_base_param_crs * obj     = OGC_NULL;
   ...
   /*---------------------------------------------------------
    * There must be 1 token: BASEPARAMCRS[ "name" ...
    */
   if ( same < 1 )
   {
   ...
   for (int i = start; i < end; i = next)
   {
           ...
           ogc_string::is_equal(arr[i].str, ogc_paramunit::alt_kwd()) )
      {
         ...
         {
            unit = ogc_paramunit::from_tokens(t, i, &next, err);
            if ( unit == OGC_NULL )
               bad = true;
         }
         continue;
 ...
 * object from WKT
 */
ogc_base_param_crs * ogc_base_param_crs :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_base_param_crs * obj = OGC_NULL;
 ...
 * object to WKT
 */
bool ogc_base_param_crs :: to_wkt(
 ...
 * clone
 */
ogc_base_param_crs * ogc_base_param_crs :: clone(const ogc_base_param_crs * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_base_param_crs * ogc_base_param_crs :: clone() const
{
   ...
   ogc_base_param_crs * p = create(_name,
                                   datum,
                                   cs,
                                   axis_1,
                                   paramunit,
 ...
 */
bool ogc_base_param_crs :: is_equal(
   const ogc_base_param_crs * p1,
...
bool ogc_base_param_crs :: is_equal(
   const ogc_base_param_crs * p) const
{
 ...
 * compare
 */
bool ogc_base_param_crs :: is_identical(
   const ogc_base_param_crs * p1,
   const ogc_base_param_crs * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
