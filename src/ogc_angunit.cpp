...
const char * ogc_angunit :: obj_kwd() { return OGC_OBJ_KWD_ANGUNIT; }
const char * ogc_angunit :: alt_kwd() { return OGC_OBJ_KWD_UNIT;    }

bool ogc_angunit :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_angunit * ogc_angunit :: create(
   const char * name,
   double       factor,
   ogc_vector * ids,
   ogc_error *  err)
{
   ogc_angunit * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      ...
      p->_obj_type  = OGC_OBJ_TYPE_ANGUNIT;
      p->_unit_type = OGC_UNIT_TYPE_ANGLE;
      p->_factor    = factor;
      p->_ids       = ids;
   }
 ...
 * object from tokens
 */
ogc_angunit * ogc_angunit :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 2 tokens: ANGUNIT[ "name", factor ...
    */
   if ( same < 2 )
   {
 ...
 * object from WKT
 */
ogc_angunit * ogc_angunit :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_angunit * obj = OGC_NULL;
 ...
 * clone
 */
ogc_angunit * ogc_angunit :: clone(const ogc_angunit * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_angunit * ogc_angunit :: clone() const
{
   ogc_vector * ids = ogc_vector :: clone( _ids );

   ogc_angunit * p = create(_name,
                            _factor,
                            ids,
                            OGC_NULL);
   if ( p == OGC_NULL)
 ...
 */
bool ogc_angunit :: is_equal(
   const ogc_angunit * p1,
