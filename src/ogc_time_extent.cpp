...
const char * ogc_time_extent :: obj_kwd() { return OGC_OBJ_KWD_TIME_EXTENT; }

bool ogc_time_extent :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_time_extent * ogc_time_extent :: create(
   const char *   start,
   const char *   end,
   ogc_error *    err)
{
   ogc_time_extent * p = OGC_NULL;
   bool bad = false;
   ...
   {
      int len = ogc_string::unescape_len(start);
      if ( len >= OGC_TIME_MAX )
      {
   ...
   {
      int len = ogc_string::unescape_len(end);
      if ( len >= OGC_TIME_MAX )
      {
         ogc_error::set(err, OGC_ERR_TIME_TOO_LONG, obj_kwd(), len);
         bad = true;
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_time_extent();
      if ( p == OGC_NULL )
      {
 ...
 * destroy
 */
ogc_time_extent :: ~ogc_time_extent()
{
}
 ...
 * object from tokens
 */
ogc_time_extent * ogc_time_extent :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
    ...
    */
   start_time = arr[start++].str;
   end_time   = arr[start++].str;
   ...
   if ( !bad )
   {
      obj = create(start_time, end_time, err);
   }
 ...
 * object from WKT
 */
ogc_time_extent * ogc_time_extent :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_time_extent * obj = OGC_NULL;
 ...
 * clone
 */
ogc_time_extent * ogc_time_extent :: clone(const ogc_time_extent * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_time_extent * ogc_time_extent :: clone() const
{
   ogc_time_extent * p = create(_start, _end, OGC_NULL);
   return p;
}
 ...
 */
bool ogc_time_extent :: is_equal(
   const ogc_time_extent * p1,
   const ogc_time_extent * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
   if ( p1 == OGC_NULL || p2 == OGC_NULL ) return false;

   if ( !ogc_string :: is_equal( p1->start(), p2->start() ) ||
        !ogc_string :: is_equal( p1->end(),   p2->end()   ) )
   {
      return false;
   }
 ...
 * compare
 */
bool ogc_time_extent :: is_identical(
   const ogc_time_extent * p1,
   const ogc_time_extent * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
