...
const char * ogc_uri :: obj_kwd() { return OGC_OBJ_KWD_URI; }

bool ogc_uri :: is_kwd(const char * kwd)
{
   ...
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: URI[ "text" ...
    */
   if ( same < 1 )
   {
 ...
 * clone
 */
ogc_uri * ogc_uri :: clone(const ogc_uri * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_uri * ogc_uri :: clone() const
{
   ogc_uri * p = create(_text);
   return p;
}
