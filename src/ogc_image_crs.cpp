   ...
   ogc_error *        err)
{
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_image_crs();
      if ( p == OGC_NULL )
      {
 ...
 * object from tokens
 */
ogc_image_crs * ogc_image_crs :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: IMAGCRS[ "name" ...
    */
   if ( same < 1 )
   {
   ...
   if ( !bad )
   {
      obj = create(name, datum, cs, axis_1, axis_2, unit,
 ...
 * object from WKT
 */
ogc_image_crs * ogc_image_crs :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_image_crs * obj = OGC_NULL;
 ...
 * clone
 */
ogc_image_crs * ogc_image_crs :: clone(const ogc_image_crs * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
 ...
 */
bool ogc_image_crs :: is_equal(
   const ogc_image_crs * p1,
 ...
 * compare
 */
bool ogc_image_crs :: is_identical(
   const ogc_image_crs * p1,
   const ogc_image_crs * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
