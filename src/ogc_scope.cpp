 ...
 * object from tokens
 */
ogc_scope * ogc_scope :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
{
   ...
   /*---------------------------------------------------------
    * There must be 1 token: SCOPE[ "text" ...
    */
   if ( same < 1 )
   {
 ...
 * clone
 */
ogc_scope * ogc_scope :: clone(const ogc_scope * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_scope * ogc_scope :: clone() const
{
   ogc_scope * p = create(_text, OGC_NULL);
   return p;
}
