...
const char * ogc_time_datum :: obj_kwd() { return OGC_OBJ_KWD_TIME_DATUM; }
const char * ogc_time_datum :: alt_kwd() { return OGC_ALT_KWD_TIME_DATUM; }

bool ogc_time_datum :: is_kwd(const char * kwd)
{
 ...
 * create
 */
ogc_time_datum * ogc_time_datum :: create(
   const char *    name,
   ogc_anchor *    anchor,
   ogc_vector *    ids,
   ogc_error *     err)
{
   ogc_time_datum * p = OGC_NULL;
   bool bad = false;
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_time_datum();
      if ( p == OGC_NULL )
      {
      ...
      p->_obj_type   = OGC_OBJ_TYPE_TIME_DATUM;
      p->_datum_type = OGC_DATUM_TYPE_TIME;
      p->_anchor     = anchor;
      p->_ids        = ids;
   }
 ...
 * object from tokens
 */
ogc_time_datum * ogc_time_datum :: from_tokens(
   const ogc_token * t,
   int               start,
   int *             pend,
   ogc_error *       err)
 ...
 * object from WKT
 */
ogc_time_datum * ogc_time_datum :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_time_datum * obj = OGC_NULL;
 ...
 * clone
 */
ogc_time_datum * ogc_time_datum :: clone(const ogc_time_datum * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
 ...
 */
bool ogc_time_datum :: is_equal(
   const ogc_time_datum * p1,
 ...
 * compare
 */
bool ogc_time_datum :: is_identical(
   const ogc_time_datum * p1,
   const ogc_time_datum * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
