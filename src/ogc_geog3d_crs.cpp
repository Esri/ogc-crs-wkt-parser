...
/* ------------------------------------------------------------------------- */
/* GEOG3D CRS (3D geodetic CRS) object                                       */
/* ------------------------------------------------------------------------- */
...
const char * ogc_geog3d_crs :: obj_kwd() { return OGC_OBJ_KWD_GEOG3D_CRS; }

/*------------------------------------------------------------------------
 * create
 */
ogc_geog3d_crs * ogc_geog3d_crs :: create(
   const char *         name,
   ...
   ogc_error *          err)
{
   ...
   else if ( !ogc_utils::validate_cs(OGC_OBJ_TYPE_GEOG3D_CRS, cs,
   ...
   if ( !bad )
   {
      p = new (std::nothrow) ogc_geog3d_crs();
      if ( p == OGC_NULL )
      {
      ...
      p->_obj_type = OGC_OBJ_TYPE_GEOG3D_CRS;
      p->_crs_type = OGC_CRS_TYPE_GEOG3D;
      p->_cs       = cs;
      p->_datum    = datum;
      p->_primem   = primem;
      p->_axis_1   = axis_1;
 ...
 * destroy
 */
ogc_geog3d_crs :: ~ogc_geog3d_crs()
{
   ogc_geodetic_datum :: destroy( _datum );
}

void ogc_geog3d_crs :: destroy(
   ogc_geog3d_crs * obj)
{
   if ( obj != OGC_NULL )
   {
 ...
 * object from WKT
 */
ogc_geog3d_crs * ogc_geog3d_crs :: from_wkt(
   const char * wkt,
   ogc_error *  err)
{
   ogc_geog3d_crs * obj = OGC_NULL;
 ...
 * clone
 */
ogc_geog3d_crs * ogc_geog3d_crs :: clone(const ogc_geog3d_crs * obj)
{
   if ( obj == OGC_NULL )
      return OGC_NULL;
   return obj->clone();
...
ogc_geog3d_crs * ogc_geog3d_crs :: clone() const
{
 ...
 */
bool ogc_geog3d_crs :: is_equal(
   const ogc_geog3d_crs * p1,
 ...
 * compare
 */
bool ogc_geog3d_crs :: is_identical(
   const ogc_geog3d_crs * p1,
   const ogc_geog3d_crs * p2)
{
   if ( p1 == OGC_NULL && p2 == OGC_NULL ) return true;
